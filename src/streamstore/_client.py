...
from streamstore._retrier import Retrier, compute_backoffs, Attempt
...
class Stream:
            ...
            while True:
                ...
                try:
                    async with create_task_group() as tg:
                        ...
                        async with request_tx:
                            if len(inflight_inputs) > 0:
                                logging.info(
                                    f"flushing {len(inflight_inputs)} inflight inputs before proceeding ..."
                                )
                                ...
                                logging.info("done flushing inflight inputs")
                    ...
                    if attempt.value < max_attempts and any(
                        ...
                        logging.info(
                            f"backoffing {attempt.value}:{backoffs[attempt.value]} for {eg.exceptions}, num_tasks: {len(asyncio.all_tasks())}, all_tasks: {asyncio.all_tasks()}"
                        )
                        await asyncio.sleep(backoffs[attempt.value])
                        attempt.value += 1
                        logging.info(
                            f"retrying append session, num_tasks: {len(asyncio.all_tasks())}, all_tasks: {asyncio.all_tasks()}"
                        )
    ...
    async def read_session(
        ...
        while True:
            try:
                logging.info(f"reading from {request.start_seq_num}")
                ...
                if _grpc_retry_on(e) and attempt < max_attempts:
                    logging.info(
                        f"backoffing {attempt} after {backoffs[attempt]} for {e}"
                    )
